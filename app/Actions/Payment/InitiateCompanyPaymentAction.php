...
class InitiateCompanyPaymentAction
{
    public function __construct(protected PaymeraService $paymeraService) {}

    public function execute(int $companyRequestID): array
    {
        $companyRequest = CompanyRequest::where('id', $companyRequestID)->firstOrFail();

        // 1. التحقق من الدفع المسبق
        if (in_array($companyRequest->payment_status, ['paid', 'partial_paid'])) {
            throw new \Exception('This company registration fee has already been paid.');
        }

        // 2. التحقق من شرط الـ 72 ساعة (payment_due_date)
        if ($companyRequest->payment_due_date && now()->greaterThan($companyRequest->payment_due_date)) {

            // إلغاء أي عملية دفع معلقة لدى Paymera إن وجدت
            $pendingPayment = $companyRequest->payments()->where('status', 'pending')->first();
            ...
            $companyRequest->update(['status' => 'expired']);
        ...
        $existingPayment = $companyRequest->payments()
        ...
        $amountToPay = $companyRequest->required_deposit;

        $paymeraResponse = $this->paymeraService->createPayment(
            amount: $amountToPay,
            callbackUrl: $callbackUrl,
            triggerUrl: $triggerUrl,
            notes: "Company Application ID: {$companyRequestID}"
        );
        ...
        DB::transaction(function () use ($companyRequest, $amountToPay, $paymeraId, $paymentUrl, $paymeraResponse, $paymentUuid) {
            $companyRequest->payments()->create([
                ...
                'expires_at'         => $companyRequest->payment_due_date, // تاريخ الـ 72 ساعة
